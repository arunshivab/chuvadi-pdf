...
// PHASE: Glyph-level redaction — in-place replacement (Reject policy)
//
// A redaction may carry replacement text drawn in place of the removed glyphs.
// The replacement must be no wider than the removed span (Reject policy): a
// fitting replacement is rendered in the gap; a wider one is rejected.
...
public sealed class RedactionReplaceTests
{
    [Fact]
    public void Apply_ReplacementFits_DrawsInPlace_KeepsNeighbours()
    {
        ...
        RedactionOptions opts = new RedactionOptions
        {
            ...
            {
                new RedactionRect(0, new RectangleF(85, 696, 45, 20), "XXX"),
            },
        };
        ...
        after.Should().Contain("Secret");
        after.Should().Contain("here");
        after.Should().Contain("XXX", "the fitting replacement is drawn in place");
        after.Should().NotContain("amount", "the original glyphs are physically removed");
    }

    [Fact]
    public void Apply_ReplacementTooWide_RejectsWithRedactionException()
    {
        using MemoryStream source = BuildTextPdf("Secret amount here");
        using PdfDocument doc = PdfDocument.Open(source, leaveOpen: true);
        using MemoryStream output = new MemoryStream();
        ...
        RedactionOptions opts = new RedactionOptions
        {
            Rectangles = new List<RedactionRect>
            {
                new RedactionRect(0, new RectangleF(85, 696, 45, 20), "WWWWWWWWWWWWWWWWWWWW"),
            },
        };

        Action act = () => Redactor.Apply(output, doc, opts);
        act.Should().Throw<RedactionException>();
    }
